...
- "HPC systems typically provide login nodes, head nodes, and a set of worker nodes."
...
or enable a Two-factor authentication (2FA) system. 
...
> ## Login assumptions
>
> For the following commands, it is assumed that you have already received your
> credentials from [{{ site.remote.name }}][cenapad-user-site], changed your initial password, 
> and set up a two-factor authentication device. If you have not do that yet, please follow the [{{ site.remote.name }} guide][cenapad-first-login].
{: .callout}
...
First, you will be asked for your password, and then for a 6-digit verification code
given by the 2FA device you set up earlier.  __Note__: the characters you type after the
password or verification code prompts are not displayed on the screen. Normal output
will resume once you press `Enter`.

```{.output}
###############################################################
#            Welcome to CENAPAD-SP                            # 
#      All connections are monitored and recorded             #
#  Disconnect IMMEDIATELY if you are not an authorized user!  #
###############################################################
({{ site.remote.user }}@{{ site.remote.login }}) Password: 
({{ site.remote.user }}@{{ site.remote.login }}) Verification code: 

```

> ## {{ site.remote.name }} frontend
>
> {{ site.remote.name }} have a login server called "frontend". From this server we can jump 
> to their compute environments such as __{{ site.remote.node_name }}__. We just need to execute: 
> `ssh {{ site.remote.node }}`, and enter the same password you
already use in the steps above.
{: .callout}
...
* `{{ site.remote.frontend_prompt }} ` when the command is to be entered on a
  terminal connected to the remote login node.
* `{{ site.remote.prompt }} ` when the command is to be entered on a
  terminal connected to the remote head node.
...
```
{{ site.remote.frontend_prompt }} hostname
```
{: .language-bash}

```
{{ site.remote.frontend }}
```
{: .output}

So, we're definitely on the remote login node machine. Next, let's find out where we are
by running `pwd` to **p**rint the **w**orking **d**irectory.

```
{{ site.remote.frontend_prompt }} pwd
```
{: .language-bash}
...
{: .language-bash}
```
homelovelace  porcentual.{{ site.remote.user }}
```
{: .output}
...
other filesystems such as `{{ site.remote.homelink }}`. If they did not, your home directory may appear empty. To
double-check, include hidden files in your directory listing:

```
{{ site.remote.frontend_prompt }} ls -a
```
{: .language-bash}
```
.   .bash_history  .bashrc  .config                homelovelace  porcentual.{{ site.remote.user }}  .ssh
..  .bash_logout   .cache   .google_authenticator  .kshrc        .profile
```
{: .output}
...
[cenapad-user-site]: https://www.cenapad.unicamp.br/portal-do-usuario/guia/
[cenapad-first-login]: https://www.cenapad.unicamp.br/portal-do-usuario/guia/primeiroacesso.shtml?menu-guia=aberto
